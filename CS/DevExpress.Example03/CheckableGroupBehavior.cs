    ...
    public class CheckableGroupBehavior : Behavior<GridControl> {
        ...
        void AssociatedObject_EndGrouping(object sender, RoutedEventArgs e) {
            ...
            while(--g >= 0) {
                ...
                foreach(var item in levelItems) {
                    ...
                    var dic = (Dictionary<int, bool?>)this.AssociatedObject.GetRowState(item, true).GetValue(CheckableGroupBehavior.CheckListProperty);
                    dic[g] = b;
                    this._LocalSet = true;
                    this.AssociatedObject.GetRowState(item, true).SetValue(CheckableGroupBehavior.CheckListProperty, new Dictionary<int, bool?>(dic));
                    this._LocalSet = false;
                }
            }
        }
            ...
            if(grid.IsValidRowHandle(rowHandle) && grid.IsGroupRowHandle(rowHandle)) {
                ...
                while(++i < count) {
                    ...
                    if(grid.IsGroupRowHandle(childHandle)) {
                        var dic = (Dictionary<int, bool?>)grid.GetRowState(childHandle, true).GetValue(CheckableGroupBehavior.CheckListProperty);
                        b = dic[childLevel];
                    } else {
        ...
        protected Dictionary<int, bool?> GetUpdatedDic(int rowHandle, bool? value) {
            ...
            if(this.AssociatedObject.IsValidRowHandle(rowHandle)){
                ...
                var rowState = this.AssociatedObject.GetRowState(rowHandle, true);
        ...
        protected void WorkOutGroup(int rowHandle, bool? value) {
            var grid = this.AssociatedObject;
            var parentRowState = grid.GetRowState(rowHandle, true);
            if(grid.IsValidRowHandle(rowHandle) && grid.IsGroupRowHandle(rowHandle)) {
                int count = grid.GetChildRowCount(rowHandle);
                int i = -1;
                while(++i < count) {
                    var childHandle = grid.GetChildRowHandle(rowHandle, i);
                    if(grid.IsGroupRowHandle(childHandle)) {
                        var rowState = this.AssociatedObject.GetRowState(childHandle, true);
