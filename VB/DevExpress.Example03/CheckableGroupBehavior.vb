...
Namespace DevExpress.Example03
	...
	Public Class CheckableGroupBehavior
        ...
        Public Shared ReadOnly CheckListProperty As DependencyProperty =
            DependencyProperty.RegisterAttached("CheckList",
                                                GetType(Dictionary(Of Integer, Boolean?)),
                                                GetType(CheckableGroupBehavior),
                                                New PropertyMetadata(New Dictionary(Of Integer, Boolean?)(), AddressOf IsCheckListPropertyChanged))
        ...
        Protected Sub AttachItem(ByVal obj As Object)
            Dim iNotifyPropertyChanged = CType(obj, INotifyPropertyChanged)
            If iNotifyPropertyChanged IsNot Nothing Then
                AddHandler iNotifyPropertyChanged.PropertyChanged, AddressOf PropertyChanged
            End If
        End Sub
        ...
        Protected Sub GridLoaded(ByVal sender As Object, ByVal e As RoutedEventArgs)
            ...
            Dim collection As INotifyCollectionChanged = CType(Me.AssociatedObject.ItemsSource, INotifyCollectionChanged)
        ...
        Private Sub AssociatedObject_EndGrouping(ByVal sender As Object, ByVal e As RoutedEventArgs)
            ...
            Dim g As Integer = Me.AssociatedObject.GroupCount - 1
            Do While g >= 0
                ...
                For Each item In levelItems
                    ...
                    Dim dic = CType(Me.AssociatedObject.GetRowState(item, True).GetValue(CheckableGroupBehavior.CheckListProperty), Dictionary(Of Integer, Boolean?))
                    dic(g) = b
                    Me._LocalSet = True
                    Me.AssociatedObject.GetRowState(item, True).SetValue(CheckableGroupBehavior.CheckListProperty, New Dictionary(Of Integer, Boolean?)(dic))
                    Me._LocalSet = False
                Next item
                g = g - 1
            Loop
        ...
        Protected Sub WorkOutCell(ByVal rowHandle As Integer)
            ...
            If grid.IsValidRowHandle(groupRowHandle) AndAlso grid.IsGroupRowHandle(groupRowHandle) Then
                ...
                grid.GetRowState(groupRowHandle, True).SetCurrentValue(CheckableGroupBehavior.CheckListProperty, dic)
            ...
            If grid.IsValidRowHandle(rowHandle) AndAlso grid.IsGroupRowHandle(rowHandle) Then
                ...
                Do While i < count
                    ...
                    If grid.IsGroupRowHandle(childHandle) Then
                        Dim dic = CType(grid.GetRowState(childHandle, True).GetValue(CheckableGroupBehavior.CheckListProperty), Dictionary(Of Integer, Boolean?))
                        b = dic(childLevel)
                    Else
        ...
        Protected Function GetUpdatedDic(ByVal rowHandle As Integer, ByVal value? As Boolean) As Dictionary(Of Integer, Boolean?)
            ...
            If Me.AssociatedObject.IsValidRowHandle(rowHandle) Then
                ...
                Dim rowState = Me.AssociatedObject.GetRowState(rowHandle, True)
        ...
        Protected Sub WorkOutGroup(ByVal rowHandle As Integer, ByVal value? As Boolean)
            Dim grid = Me.AssociatedObject
            Dim parentRowState = grid.GetRowState(rowHandle, True)
            If grid.IsValidRowHandle(rowHandle) AndAlso grid.IsGroupRowHandle(rowHandle) Then
                Dim count As Integer = grid.GetChildRowCount(rowHandle)
                Dim i As Integer = 0
                Do While i < count
                    Dim childHandle = grid.GetChildRowHandle(rowHandle, i)
                    If grid.IsGroupRowHandle(childHandle) Then
                        Dim rowState = Me.AssociatedObject.GetRowState(childHandle, True)
